...
$Client = Read-Host -Prompt "Enter the 3-letter name of the client (ex. CFI)"
$viron = Read-Host -Prompt "Enter the environment shortname (ex. PROD, QA, etc.)"
$i=1
$cd=1
$cm=1
$sql=1
...
$rule1 = New-AzureRmNetworkSecurityRuleConfig -Name PSR -Description "Allow PSR" -Access Allow -Protocol Tcp -Direction Inbound -Priority 101 `
-SourceAddressPrefix Internet -SourcePortRange * `
-DestinationAddressPrefix * -DestinationPortRange 5986

$NSGname = "SITECORE-NSG"

$nsg = New-AzureRmNetworkSecurityGroup -ResourceGroupName $rgname -Location $rgloc -Name $NSGname -SecurityRules $Rule1

#-SecurityRules $rule1,$rule2



Set-AzureRmVirtualNetworkSubnetConfig -VirtualNetwork $virtnet -Name $subnetname -AddressPrefix $AddressPrefix -NetworkSecurityGroup $nsg

Set-AzureRmVirtualNetwork -VirtualNetwork $virtnet

$NSGID = Get-AzureRmNetworkSecurityGroup | ? Name -Like "*$NSG*" | select ID
...
$title = "Content Delivery Deployment" 
$message = "Do you want to deploy a CD Node?"
#Run the prompt
...
$i=0
   ...
   $nic = New-AzureRmNetworkInterface -Name $nicName -ResourceGroupName $rgName -Location $rgloc -SubnetId $virtnet.Subnets[0].Id -PublicIpAddressId $pip.Id -NetworkSecurityGroupId $NSGID
    
   #Prompts for local admin credentials that will be assigned as the primary local admin 
   $localadmin = Get-Credential -Message "Username and password of the new local admin for the CD Nodes. DO NOT USE ADMINISTRATOR"
...
#Titles for the choice prompt
$title = "Content Management Deployment" 
$message = "Do you want to deploy a CM Node?"
#Run the prompt
$result4 = $host.ui.PromptForChoice($title, $message, $options, 1)
switch ($VMquestion) {
    0{
        Write-Host "Yes"
    }1{
...
write-host "!!! You have chosen $vmsize !!!" -foregroundcolor "Yellow" `n
   ...
   #Prompts for local admin credentials that will be assigned as the primary local admin 
   $localadmin = Get-Credential -Message "Username and password of the new local admin for the CM Nodes. DO NOT USE ADMINISTRATOR"
   #Creates the new VM configuration and assigns to variable
   $vm = New-AzureRmVMConfig -VMName $vmName2 -VMSize $vmsize
   #Updates variable with operating system information
   $vm = Set-AzureRmVMOperatingSystem -VM $vm -Windows -ComputerName $vmName2 -Credential $localadmin -ProvisionVMAgent -EnableAutoUpdate
   #Updates variable with source image info (server 2012 R2)
   $vm = Set-AzureRmVMSourceImage -VM $vm -PublisherName MicrosoftWindowsServer -Offer WindowsServer -Skus 2012-R2-Datacenter -Version "latest"
   #Updates variable with the network interface info
   $vm = Add-AzureRmVMNetworkInterface -VM $vm -Id $nic.Id
   ...
   #Assigns a path to the blob in created "vhds" container with a vhd name of the VMname + OSdisk.vhd
   $blobPath = "vhds/$($vmName2)OSdisk.vhd"
   #Assigns blob path to the variable
   $osDiskUri = $storageAccount.PrimaryEndpoints.Blob.ToString() + $blobPath
   #Assigns vhd name (excluding path and .vhd) to the variable
   $diskName = "$($vmName2)OSdisk"
   #Updates variable to include the OS disk information
   $vm = Set-AzureRmVMOSDisk -VM $vm -Name $diskName -VhdUri $osDiskUri -CreateOption fromImage
...
#Titles for the choice prompt
$title = "SQL Deployment" 
$message = "Do you want to deploy a SQL Node?"
#Run the prompt
$result5 = $host.ui.PromptForChoice($title, $message, $options, 1)
switch ($VMquestion) {
    0{
        Write-Host "Yes"
    }1{
...
write-host "!!! You have chosen $vmsize !!!" -foregroundcolor "Yellow" `n
 ...
   
   ...
   #Prompts for local admin credentials that will be assigned as the primary local admin 
   $localadmin = Get-Credential -Message "Username and password of the new local admin for the SQL Nodes. DO NOT USE ADMINISTRATOR"
   #Creates the new VM configuration and assigns to variable
   $vm = New-AzureRmVMConfig -VMName $vmName3 -VMSize $vmsize 
   #Updates variable with operating system information
   $vm = Set-AzureRmVMOperatingSystem -VM $vm -Windows -ComputerName $vmName3 -Credential $localadmin -ProvisionVMAgent -EnableAutoUpdate
   #Updates variable with source image info (server 2012 R2)
   $vm = Set-AzureRmVMSourceImage -VM $vm -PublisherName MicrosoftWindowsServer -Offer WindowsServer -Skus 2012-R2-Datacenter -Version "latest"
   #Updates variable with the network interface info
   $vm = Add-AzureRmVMNetworkInterface -VM $vm -Id $nic.Id
   ...
   #Assigns a path to the blob in created "vhds" container with a vhd name of the VMname + OSdisk.vhd
   $blobPath = "vhds/$($vmName3)OSdisk.vhd"
   #Assigns blob path to the variable
   $osDiskUri = $storageAccount.PrimaryEndpoints.Blob.ToString() + $blobPath
   #Assigns vhd name (excluding path and .vhd) to the variable
   $diskName = "$($vmName3)OSdisk"
   #Updates variable to include the OS disk information
   $vm = Set-AzureRmVMOSDisk -VM $vm -Name $diskName -VhdUri $osDiskUri -CreateOption fromImage
